...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var schedulings = JSON.parse($('#schedulings').val());
  var paciente_meus_agendamentos = new FullCalendar.Calendar(c2, {
    ...
    events: schedulings,
      eventClick: function(info) {
        ...
        $('.name').html(info.event.extendedProps.special+ ' - ' + info.event.extendedProps.name)
        $('.start').html($.date(info.event.start))
        $('.end').html($.date(info.event.end))
        $('.subject').html(info.event.extendedProps.subject)
        $('.description').html(info.event.extendedProps.description)
        $('.office').html(info.event.extendedProps.office)
        $('.address').html(info.event.extendedProps.address)
...
$('#section-counter .container').show();
