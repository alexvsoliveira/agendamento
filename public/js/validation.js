$(document).ready(function () {
    ...
    $(document).on('click', '.agendar-consulta', function(){
        var special_id =  $(this).attr('data-special');
        var office_id =  $(this).attr('data-office');
        var user_id =  $(this).attr('data-user');
        var professional_id =  $(this).attr('data-professional');

        $('.confirmAgendamentoBt').attr('data-special', special_id);
        $('.confirmAgendamentoBt').attr('data-office', office_id);
        $('.confirmAgendamentoBt').attr('data-user', user_id);
        $('.confirmAgendamentoBt').attr('data-professional', professional_id);
        $('#confirmar_agendamento').modal('show');
    });

    $('.fechar_final').click(function(){
        location.reload();
        return false;
    })

    $('.confirmAgendamentoBt').click(function(){
        var special_id =  $(this).attr('data-special');
        var office_id =  $(this).attr('data-office');
        var user_id =  $(this).attr('data-user');
        var professional_id =  $(this).attr('data-professional');
        ...
        data = {
            ...
            'data_consulta': $('#busca_dtConsulta').val(),
            'horario_consulta': $('#busca_horario_paciente').val(),
            'subject': $('#subject').val(),
        ...
        $.ajax({
            ...
            success: function(data){
                $('#confirmado_agendamento').modal('show');
                $('#confirmar_agendamento').modal('hide');
            },
            error: function(){
    ...
    $(".buscar").click(function () {
        ...
        var data = {
            special_id: $('#busca_especialidade').val(),
            data_consulta: $('#busca_dtConsulta').val(),
            horario_consulta: $('#busca_horario_paciente').val()
        };
        ...
        $.ajax({
            ...
            type: 'post',
            url: "/get-scheduling-open",
            data: data,
            success: function(data){
                if(data.length > 0){
                    $('.append-result').html('');
                    for(var i in data){
                        let proName = data[i].sexo == 'm' ? 'Dr.' : 'Dra.';
                        let image = data[i].profile_photo_path == null ? '/images/icon.png' : $('#url_base').val()+'/storage/'+data[i].profile_photo_path
                        let body = '<div class="col-lg-4 col-md-6 col-sm-6">'+
                            '<div class="box">'+
                                '<div class="icon">'+
                                    '<img src="'+image+'" class="rounded-full h-20 w-20 object-cover" alt="">'+
                                '</div>'+
                                '<div class="especialidade mt-2">'+data[i].special+'</div>'+
                                '<div class="nome-doutor">'+proName+' '+data[i].name+'</div>'+
                                '<div class="endereco mt-3"><span class="icon-business"></span> '+data[i].office+'</div>'+
                                '<div class="endereco mt-3"><span class="icon-map"></span> '+data[i].address+'</div>'+
                                '<a data-office="'+data[i].office_id+'" data-user="'+$('#user_id').val()+'" data-professional="'+data[i].user_id+'" data-special="'+data[i].special_id+'" style="float:none;" class="btn btn-secondary px-4 mt-4 agendar-consulta">Agendar</a>'+
                            '</div>'+
                        '</div>';
